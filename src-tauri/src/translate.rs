//! Tradução offline (pt-BR · es · en) com modelos Marian/OPUS-MT da Helsinki-NLP
//! rodando no candle (CPU, Rust puro — sem runtime externo, sem sidecar).
//!
//! Motor extraído do LocalZIM (mesma release de modelos). Os bundles convertidos
//! (f16 + tokenizer.json) são publicados como assets da release `v1` do repo
//! Anon5T4R/LocalZIM-models; o app baixa cada direção sob demanda para
//! `app_data/translate/models/<direção>/`, com sha256 verificado durante o
//! download. pt↔es não tem modelo dedicado da Helsinki: o app pivota pelo inglês
//! (duas pernas, dois modelos).
//!
//! Diferença vs LocalZIM: aqui não há artigo/UUID — o texto vem das duas caixas
//! da UI e o histórico fica no SQLite; o motor só traduz um lote de linhas.
...
pub fn legs(direction: &str) -> Option<Vec<&'static str>> {
    match direction {
...
/// Todas as direções que a UI oferece (as 6 combinações dos 3 idiomas).
pub const ALL_DIRECTIONS: [&str; 6] = ["en-pt", "pt-en", "en-es", "es-en", "pt-es", "es-pt"];
...
pub fn direction_status(app_data: &Path, tr: &Translator, direction: &str) -> Option<DirectionStatus> {
    ...
    let legs: Vec<LegStatus> = legs(direction)?
        .into_iter()
        .map(|leg| LegStatus {
            leg: leg.to_string(),
    ...
    Some(DirectionStatus {
        ...
        installed: legs.iter().all(|l| l.installed),
        downloading: legs.iter().any(|l| l.downloading),
        bytes: legs.iter().map(|l| l.bytes).sum(),
        legs,
    })
}

pub fn all_status(app_data: &Path, tr: &Translator) -> Vec<DirectionStatus> {
    ALL_DIRECTIONS
        .iter()
        .filter_map(|d| direction_status(app_data, tr, d))
        .collect()
}
...
fn engine_for(app_data: &Path, tr: &Translator, leg: &str) -> Result<Arc<Mutex<Engine>>, String> {
...
/// Traduz um lote de blocos de texto (uma linha por bloco, preservando a
/// estrutura de parágrafos da UI). Só o que tem letra passa pelo modelo,
/// pivotando quando a direção precisa de duas pernas.
pub fn translate_texts(
    ...
    let mut out: Vec<Option<String>> = texts
        .iter()
        .map(|t| (!needs_translation(t)).then(|| t.clone()))
        .collect();
...
/// Traduz um texto multi-linha inteiro, preservando as quebras de linha.
pub fn translate(
    app_data: &Path,
    tr: &Translator,
    direction: &str,
    ...
    let out = translate_texts(app_data, tr, direction, lines)?;
    Ok(out.join("\n"))
}
...
mod tests {
    ...
    #[test]
    fn todas_as_direcoes_tem_pernas_validas() {
        for d in ALL_DIRECTIONS {
            let ls = legs(d).unwrap_or_else(|| panic!("direção {d} sem pernas"));
            assert!(ls.len() == 1 || ls.len() == 2);
            for leg in ls {
                assert!(manifest().directions.contains_key(leg), "perna {leg} sem manifesto");
            }
        }
    }

    /// De ponta a ponta com modelo real (não roda no CI): aponte
    /// LOCALTRANSLATE_TEST_APPDATA pra uma pasta com translate/models/en-es/
    /// preenchida com um bundle convertido.
    #[test]
    fn traducao_de_ponta_a_ponta_com_bundle_local() {
        let Ok(app_data) = std::env::var("LOCALTRANSLATE_TEST_APPDATA") else {
            eprintln!("LOCALTRANSLATE_TEST_APPDATA não definido — teste pulado");
            return;
        };
        ...
        let out = translate(
            ...
            &tr,
            "en-es",
            "The library contains the entire Wikipedia.\n[1]",
        )
        .expect("tradução falhou");
        let lines: Vec<&str> = out.split('\n').collect();
        assert_eq!(lines.len(), 2);
        assert!(lines[0].to_lowercase().contains("wikipedia"), "saída: {}", lines[0]);
        assert_eq!(lines[1], "[1]"); // sem letras: intacto
    }
}
